#' @title Constructor for NGBoost Classifier models.
#' @description
...
#' @export
NGBClassifier <- R6::R6Class(
  classname = "NGBClassifier",
  public = list(
    #' @description Initialize NGBoost Classifier model.
    ...
    initialize = function(Dist = NULL,
      ...
      private$model <- ngboost$NGBClassifier(
        Dist = private$Dist,
        Score = private$Score,
        Base = private$Base,
    ...
    #' @description Probability prediction of Y at the points X=x
    #' @param X DataFrame object or List or numpy array of predictors (n x p)
    #' in numeric Format
    #' @param max_iter Get the prediction at the specified number of boosting
    #' iterations
    #' @return Numpy array of the estimates of Y
    #'
    predict_proba = function(X, max_iter = NULL){
      model = private$model
      model$predict_proba(X = X, max_iter = max_iter)
    },
    ...
    #' @return Numpy array of the estimates of Y
    #'
    predict_pred_dist = function(X, max_iter = NULL){
      model = private$model
    ...
    #' @param max_iter get the prediction at the specified number of boosting
    #'  iterations.
    #' @return A NGBDistClass Class
    #'
    #' @details See for available methods \code{\link{NGBDistClass}}
    #'
    pred_dist = function(X, max_iter=NULL){
      model = private$model
      NGBDistClass$new(model$pred_dist(X = X, max_iter=max_iter))
    }
  ),
